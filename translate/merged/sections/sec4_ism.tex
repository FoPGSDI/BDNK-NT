...
% Merged from: agent_12.tex (S4 intro, S4.1, S4.2 start),
%              agent_13.tex (S4.2 continued),
%              agent_14.tex (S4.2 end, S4.3, S4.4, S4.5 start),
%              agent_16.tex (S4.7 end, S4.8),
%              agent_17.tex (S4.8 end, S4.9)
% NOTE: Sections 4.5 (end), 4.6, 4.7 (beginning) are missing (no agent_15).
% =============================================================================
...
curves do not intersect, then the two equations are incompatible for that $\dot{M}_0$, and
...
\item The ellipse and hyperbola intersect twice for every value of $r$
  (Figure~\ref{fig:4.2.2}a). In this case we have 2 separate curves,
  corresponding to 2 families of intersection points.

\item The ellipse and hyperbola meet tangentially (Figure~\ref{fig:4.2.2}b)
  at one value of $r$ (call it $r_s$), at which the two families of intersection
  points $u(a)$ cross each other. We shall show below that this crossing point
  necessarily lies on the ``bisectrix'' of the graph, $u = a$.
\end{enumerate}
...
which some stars eject matter into interstellar space; see Chapter~13 of ZN.}
because accretion requires $u_\infty = 0$, $a_\infty \neq 0$. The lower
curve $u(a)$ begins at the point $u_\infty = 0$, $a_\infty \neq 0$ ---
...
of~$r$.  Dashed curves: hyperbolae from the continuity equation.
Heavy dots mark intersection points; the locus of these intersections
traces the flow solution $u(a)$.  The diagonal $u = a$ is the
``bisectrix''.  Regions above and below the bisectrix are labelled
supersonic and subsonic respectively.}}
...
We must still decide which type of lower curve is reasonable---one that remains
always below the bisectrix (case~1), or one that crosses the bisectrix (case~2).
In case~1 the flow is subsonic at all radii, $u < a$. But this requires a large
back-pressure at all radii to retard the inflow---back pressure that can never
be provided near the gravitational radius of a black hole. The gas must cross
...
field.  (a)~[case~1 in text] corresponds to flow that remains subsonic
or supersonic everywhere.  (b)~[case~2 in text] corresponds to flow for
which there is a sonic point, $u = a$.}}
...
crucial to the accretion process.  It governs the accretion rate
$\dot{M}_0$, in the same manner as the transition to supersonic flow in the
throat of a rocket nozzle.  In both cases one and only one mass flow rate is
compatible with the required transition to supersonic flow.

Let us calculate the required accretion rate $\dot{M}_0$.  We begin by
calculating the flow velocity at the transition radius (``sonic radius'')
$r_s$.  To do this, we rewrite the Euler equation~(4.2.2) in the form
\begin{equation}
\frac{du}{dr} = \frac{a^2}{r} \cdot \frac{GM/r - 2a^2}{u^2 - a^2}\,.
\tag{4.2.5}
\end{equation}
...
\frac{dp}{dr}\,\frac{(u^2 - a^2)}{p} =
  -\frac{GM}{r^2}\,\frac{2u^2}{r} + \frac{GM}{r^2}\,,
\tag{4.2.6}
\end{equation}
%
...
\begin{equation}
\dot{M}_0
= 4\pi\alpha\,G^2 M^2\,\rho_\infty / a_\infty^3
= 4\Gamma^{3/2}\,\alpha\,G^2\,M^2\,\rho_\infty\,(m_p/kT_\infty)^{3/2},
\tag{4.2.10}
\end{equation}
%
where
\begin{equation}
p_\infty = K\rho_\infty^\Gamma,
\qquad
a_\infty^2 = \Gamma\,K\,\rho_\infty^{\Gamma-1}
            = \Gamma\,p_\infty/\rho_\infty
            = \Gamma\,k\,T_\infty/(m_p\,\mu'),
\end{equation}
%
...
%
\begin{alignat}{3}
&\alpha \simeq 1.5  &\qquad &\text{for} &\qquad &\Gamma = 1, \notag \\
&\alpha \simeq 1.2  &\qquad &\text{for} &\qquad &\Gamma = 1.4, \notag \\
&\alpha \simeq 0.3  &\qquad &\text{for} &\qquad &\Gamma = 5/3;
\end{alignat}

This expression for the accretion rate is the main result of our calculation.

It is now straightforward to derive from equations~(4.2.3) and~(4.2.4)
the radial distributions of all quantities.  The general picture is as
follows.  Outside the ``radius of influence''
\begin{equation}
r_i = 2\,GM/a_\infty^2
    = r_g\,(c/a_\infty)^2
\tag{4.2.12}
...
\begin{equation}
u = (2GM/r)^{1/2} = a_\infty\,(r_i/r)^{1/2} \quad \text{at} \quad r < r_s\,;
\tag{4.2.13a}
\end{equation}
%
...
\rho = \frac{\dot{M}_0}{4\pi r^2 u}
     \propto \frac{\rho_\infty}{r^2}
       \left(\frac{r_i}{r}\right)^{3/2} \quad
\text{at} \quad r < r_s\,.
\tag{4.2.13b}
\end{equation}
...
\begin{equation}
T \simeq 0.5\,T_\infty\left(\frac{r_i}{r}\right)^{1/2(\Gamma-1)}
\quad \text{if } \Gamma = 1.4 \text{ near sonic point.}
\tag{4.2.13c}
\end{equation}
...
\begin{equation}
\dot{M}_0 = \pi r_s^2\,\rho_\infty\,c\,(a_\infty/a_s)^3.
\tag{4.2.14}
\end{equation}
...
accretion rate is larger by a factor
$(c/a_\infty)^2 \simeq 10^{10}$ than the accretion rate for independent
particles.  The physical reason for this is clear: gas is distinguished from
independent particles by the frequent collisions of its atoms; these
collisions limit the tangential velocities during infall, but permit radial
velocities to grow.
...
= \bigl(5 \times 10^{20}~\text{ergs/(g\,sec)}\bigr)\,
  ...
  \left(\frac{r_i}{r}\right)^{\!-(3/2)(\Gamma-1)}
\end{equation}
%
...
to strongly influence the flow, we can use equation~(4.2.15) for $\dot{M}_0$ and write
%
\begin{equation}
\tag{4.4.3a}
\rho
...
% NOTE: The text of Section 4.5 is incomplete here. Agent 14 ends mid-sentence.
% Sections 4.5 (conclusion), 4.6, and 4.7 (beginning) are missing (no agent_15 available).

%% ====================================================================
%% 4.7 Gas Flow Near the Horizon (continuation from agent_16)
%% ====================================================================

% [Section 4.7 beginning is missing; the following is the continuation from agent_16]

\subsection{Gas Flow Near the Horizon}
\label{sec:4.7}

% [Beginning of section not available -- text continues mid-section]
...
\caption{The trajectories of test particles~(a), and the flow lines of supersonic gas~(b)
...
More careful calculations give accretion rates which differ from this by multiplicative factors of $\sim 0.5$ to 1.0 (Bondi and Hoyle 1944)\cite{BondiHoyle1944}.
...
Sunyaev~1971). A variety of plasma instabilities may also arise. And the recon\-nection
of magnetic field lines will surely be important. One can only guess that
the overall, time-averaged equipartition model of
Schwartzman~(\S\,4.4)\cite{Schwartzman1971} applies.

One factor that may be important near the gravitational radius, where most
of the luminosity is produced, is angular momentum. Suppose that the accreting
gas has nonzero angular momentum per unit mass. If the angular momentum per unit mass, $\mathscr{L}$, exceeds
...
produced (Salpeter 1964; Schwartzman 1971)\cite{Salpeter1964,Schwartzman1971}. Almost nothing is known today
about these issues.
...
\begin{equation}
2 b_{\text{capture}} = \frac{2r_g c^2}{u_\infty^2} = (3 \times 10^{14}\;\text{cm})
...
\begin{equation}
\mathscr{L}_{\text{turb}} \simeq 1 \times 10^{20}\;\text{cm}^2\;\text{sec}^{-1}
